...
class Scheduler(Node):
    def __init__(self):
        ...
        self.notify_service = self.create_service(Empty,'notify_arrival',self.notify_arrival_callback)
        ...
        while not self.set_goal_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('set goal service not available, waiting again ...')
